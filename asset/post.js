  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      for (let i = 0; i < posts.length; i++) {
        postDiv.innerHTML += "<h2>Post for: " + posts[i].post_type + "</h2>";
        postDiv.innerHTML +=
          "<h3>" +
          posts[i].title +
...
function addPost() {
  ...
  let menu_item_id = document.getElementById("menu_item_id").value;
  ...
  xhttp.send("type=addPost&post=" + encodeURIComponent(data));
}
